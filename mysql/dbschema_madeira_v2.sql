...
CREATE TABLE IF NOT EXISTS `madeira`.`transport` (
  ...
  CONSTRAINT `fk_transport_transportype_id`
...
SHOW WARNINGS;
CREATE INDEX `idx_station_companyid` ON `madeira`.`station` (`companyid` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `madeira`.`operationalperiod` (
  ...
  `startperiod` DATE NULL COMMENT 'Start date of the operarional period',
  `endperiod` DATE NULL COMMENT 'End date of the operational period',
